...
# --- File paths required for GAIA report
...
python "$SCRIPT_DIR/../../run_evaluation.py" \
    ...
    $USE_CONTEXT_MANAGER_FLAG \
    "${LLM_JUDGE_ARGS[@]}" \
    --run-gaia-specific-report "$RUN_GAIA_SPECIFIC_REPORT" \
    --gaia-metadata-file "$GAIA_METADATA_FILE" \
    --gaia-text-data-file "$GAIA_TEXT_DATA_FILE" \
    --gaia-reports-dir "$GAIA_REPORTS_DIR" \
    --k "$PASS_K" \
