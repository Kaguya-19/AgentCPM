...
def main():
    ...
    model:  Qwen3ForCausalLM = AutoModelForCausalLM.from_pretrained(
        ...
        torch_dtype=torch.bfloat16,
        trust_remote_code=True).to(device="cuda:{}".format(os.environ.get("LOCAL_RANK", "0")))
    tp_size = 8
    mesh = dist.device_mesh.init_device_mesh(
        "cuda",
        mesh_shape=(int(os.environ.get("LOCAL_WORLD_SIZE",1))//tp_size, 1,tp_size),
    ...
    optimizer = torch.optim.AdamW(model_2d.parameters(), lr=1e-5, foreach=True)
    ...
    for _ in range(10):
        data = [{
            "prompt": [
                {"role":"user", "content": ["What is the capital of"*3000]},
            ],
