...
# 配置日志
logger.add("bing_benchmark_results.log", rotation="100 MB", level="INFO")
...
class BingBenchmark:
    ...
    async def _make_request(self, query: str) -> Tuple[bool, float, Dict]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                # 检查结果是否为空
                if "webPages" not in data or "value" not in data["webPages"] or not data["webPages"]["value"]:
                    return True, response_time, {"results": [], "empty": True}
                
                return True, response_time, {"results": data["webPages"]["value"], "empty": False}
    ...
    async def run(self, concurrency: int = 10):
        ...
        self.console.print(Panel(f"[bold green]开始Bing搜索引擎直接API压力测试[/bold green]\n"
                                f"API密钥: {self.api_key[:8]}...{self.api_key[-4:] if len(self.api_key) > 12 else ''}\n"
                                f"目标RPM: {self.rpm}\n"
                                f"持续时间: {self.duration}秒\n"
                                f"并发数: {concurrency}\n"
                                f"每次返回结果数: {self.count}"))
    ...
    def _show_results(self):
        ...
        # 创建结果表格
        table = Table(title="Bing搜索引擎API压力测试结果")
    ...
    def _save_results(self):
        ...
        filename = f"bing_benchmark_result_{timestamp}.json"
...
async def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="Bing搜索引擎API直接压力测试工具")
    parser.add_argument("--api-key", required=True, help="Bing API密钥")
    ...
    parser.add_argument("--count", type=int, default=5, help="每次搜索返回的结果数量")
