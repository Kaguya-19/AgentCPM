...
"""
AgentDock Node Full - Simple MCP Server
    ...
    def do_POST(self):
        ...
        if path == '/mcp':
            ...
            # Handle tool call request
                ...
                tool_args = params.get("arguments", {}) or params.get("params", {})
        ...
        # Handle SSE message request
            ...
            if not session_id:
                self.send_json_response({"error": "Missing session ID"}, 400)
                return
            
            if session_id not in sessions:
                self.send_json_response({"error": "Invalid session ID"}, 404)
                return
...
def cleanup_sessions():
    ...
    while True:
        ...
        inactive_sessions = [sid for sid, s in sessions.items() if now - s["last_active"] > inactive_timeout]
