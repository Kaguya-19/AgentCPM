...
`;
        ...
        const packageInfo = JSON.parse(
            (typeof stdout === "string" ? stdout : stdout.toString()).trim()
          );
        ...
        tools: [
            {
                ...
                Packages installed in the environment: PyPDF2 geopy PyMuPDF docx2txt pdfminer.six rdkit python-chess stockfish yfinance CoolProp seaborn python-pptx python-docx pdfplumber geopandas biopython pubchempy googletrans pyshp selenium waybackpy networkx wbdata 
                and their dependencies and other common python packages` ,
                inputSchema: {
                    type: "object",
                    properties: {
                        code: {
            ...
            //     description: `Check if packages are installed in the ${ENV_CONFIG.type} environment, if the packages you need were not installed, 
            //     just raise an error between the answer tags.`,
            //     inputSchema: {
            //         type: "object",
            //         properties: {
            //             packages: {
